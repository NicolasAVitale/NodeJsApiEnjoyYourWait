...
class FilaClientesDaoDb extends FilaClientesDao {
    ...
    async removeClient(queueClient) {
        let result
        try {
            result = await this.client.removeClientToQueue(queueClient, this.tabla)
            return result
        } catch (err) {
            throw new CustomError(500, 'error al remover cliente de la fila', err)
        }
    }
